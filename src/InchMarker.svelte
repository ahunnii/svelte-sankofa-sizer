<script>
  ...
  export let pixels_per_inch = 96; // typical screen dpi
	...
	export let display_footer = true;
	export let bg_color = 'rgba(255,255,255, 0.5)'
	let text_height = 1;
  ...
  let minor_ticks = [];
	...
	const num_notches = 16;
	
	$: {
	    major_ticks = Array.from({ length: length + 1 }, (_, i) => i * pixels_per_unit).filter(tick => tick <= ruler_length);
	    minor_ticks = unit === "inches"
	      ? Array.from({ length: length * num_notches }, (_, i) => i * pixels_per_unit / num_notches).filter(tick => tick <= ruler_length)
